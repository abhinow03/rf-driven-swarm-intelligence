...
### Step 1: teacher regeneration — `NVIDIA_API_KEY` provided mid-session, corpus regenerated for real

`NVIDIA_API_KEY` was not present in the environment when this review began (no env var, no
`.env` file anywhere in the repo or home directory — checked via presence-only tests, values
never printed). The user supplied a working key directly. Handled per `CLAUDE.md`'s "secrets
come from the environment, never commit a key" convention: passed inline to the subprocess
environment for the generation command only, never written to any file, never logged (verified
after the fact: zero occurrences of the key substring in any log file this session produced).

**First attempt (`--concurrency 3`, the script's implicit default from Phase 1 precedent) used
the teacher on only 119/1000 rows (11.9%)** — a single direct `NvidiaClient.generate()` call
succeeded immediately, ruling out an auth/model/endpoint problem; the low rate was concurrent-
request throttling exhausting the 3-attempt/exponential-backoff retry in
`_generate_one_with_retry` under concurrent load. **Re-ran at `--concurrency 3` explicitly**
(down from the script's true default, which is actually higher — the first run's low number was
the smoking gun that the default concurrency exceeds what this key/endpoint tolerates under
sustained load): **999/1000 rows (99.9%) used the teacher**, one `oscillation` row fell back to
templated prose after exhausting retries. 85.2 minutes wall-clock (vs 8.2 minutes at the
higher, throttled concurrency) — the real cost of respecting the rate limit instead of masking
it with silent fallback.

`data/abstention_corpus_teacher.jsonl` (999/1000 teacher-authored) is a SEPARATE file from
`data/abstention_corpus.jsonl` (the original, 0/1000 teacher, already gate-passed) — neither
overwrites the other, both exist on disk pending the merge decision.

**All 4 hard gates re-run against the teacher corpus
(`python scripts/phase3a_step4_gates.py --corpus data/abstention_corpus_teacher.jsonl --meta
data/abstention_corpus_teacher_meta.json --out-suffix _teacher`, the gates script generalized
with `--corpus`/`--meta`/`--out-suffix` args rather than a second copy of the gate logic) — ALL
PASS**, identical structure to the original no-teacher gate run: row counts exact
(780/120/100), zero overlap with the 12,001 RULES rows, zero internal duplicates, 0/60
spot-checked rows mislabeled (multi_hop/oscillation re-verified from chain alone;
terminal_transitioning's 20-row spot-check is disclosed as not re-verifiable from the persisted
chain alone, same limitation as the original run — real, not silently passed), safety copy
re-confirmed byte-identical and read-only. Teacher prose quality is a merge-time reviewer
judgment call this section does not make; the label/gate integrity is unaffected by which prose
path a row took, confirmed structurally, not assumed.
